...
export function FoodForm({ action, categories, food, mode }: FoodFormProps) {
  ...
  return (
    <Card className="mx-auto w-full max-w-2xl" key={state.submissionId ?? `${mode}-${food?.id ?? "new"}`}>
      <CardHeader>
        <CardTitle>{isEdit ? "Edit Menu" : "Tambah Menu"}</CardTitle>
        <CardDescription>{isEdit ? "Perbarui nama, harga, deskripsi, dan status menu." : "Menu akan tampil di menu meja milik owner ini."}</CardDescription>
      </CardHeader>
      <form action={formAction}>
        <CardContent>
          <FieldGroup>
            {isEdit && food ? <input name="id" type="hidden" value={food.id} /> : null}
            <Field data-invalid={!!categoryError}>
              ...
              <Select defaultValue={defaultCategory} name="category_id">
                <SelectTrigger aria-invalid={!!categoryError} className="w-full" id="category_id">
            ...
            <Field data-invalid={!!priceError}>
              ...
              <Input aria-invalid={!!priceError} defaultValue={defaultPrice} id="price" min={0} name="price" required step={500} type="number" />
              <FieldError>{priceError}</FieldError>
            </Field>
            <Field data-invalid={!!descriptionError}>
              <FieldLabel htmlFor="description">Deskripsi</FieldLabel>
              <Textarea aria-invalid={!!descriptionError} defaultValue={defaultDescription} id="description" name="description" placeholder="Opsional" />
            ...
            <Field data-invalid={!!imageError}>
              ...
              {food?.imageUrl ? (
                <div className="relative aspect-video overflow-hidden rounded-md border bg-muted">
                  <Image alt={food.name} className="object-cover" fill sizes="(max-width: 768px) 100vw, 672px" src={food.imageUrl} unoptimized />
                </div>
              ) : null}
              <Input accept="image/*" aria-invalid={!!imageError} id="image" name="image" type="file" />
              <FieldDescription>Opsional. Gunakan gambar maksimal 1 MB.</FieldDescription>
              <FieldError>{imageError}</FieldError>
            </Field>
            <Field orientation="horizontal">
              <Checkbox defaultChecked={defaultAvailability} id="is_available" name="is_available" />
              <FieldContent>
                <FieldLabel htmlFor="is_available">Tersedia</FieldLabel>
                <FieldDescription>Tampilkan menu di menu pelanggan.</FieldDescription>
              </FieldContent>
            </Field>
            {state.message ? (
